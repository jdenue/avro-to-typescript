...
export class ClassConverter extends RecordConverter {
    ...
    public convert(data: any): any {
        ...
        this.importRows.push(...this.extractImports(data.namespace));
    ...
    protected extractImports(namespace: string): string[] {
        const rows: string[] = [];

        rows.push(`// tslint:disable`);
        rows.push(`import { BaseAvroRecord } from "` + "../".repeat(namespace.split(".").length) + `BaseAvroRecord";`);
